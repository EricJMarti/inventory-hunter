...
from scraper.microcenter import MicroCenterScrapeResult as ScrapeResult
...
class InStockFixture(unittest.TestCase):
    ...
    def test_price(self):
        self.assertEqual(self.result.price, 249.99)


class InStockStoreOnlyFixture(unittest.TestCase):
    def setUp(self):
        self.result = load_result('in_stock_store_only.html')

    def test_in_stock(self):
        self.assertTrue(self.result)

    def test_price(self):
        self.assertEqual(self.result.price, 2299.99)
